...
export function SearchFilters({ filters, onChange, onClose }: SearchFiltersProps) {
  ...
  const [remoteSuggestions, setRemoteSuggestions] = useState<ViaCepAddress[]>([])
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            placeholder="Busque por rua, bairro ou localidade"
          ...
          {showNeighborhoodSuggestions && (loadingSuggestions || remoteSuggestions.length > 0) && (
            <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg">
              ...
              {!loadingSuggestions && remoteSuggestions.map(item => (
                <button
                  key={`${item.cep}-${item.logradouro}-${item.bairro}`}
                  type="button"
                  onMouseDown={event => {
                    event.preventDefault()
                    setNeighborhoodQuery(item.bairro)
                    setShowNeighborhoodSuggestions(false)
                    void (async () => {
                      try {
                        const coordinates = await geocodeCepAction(item.cep)
                        onChange({
                          ...filters,
