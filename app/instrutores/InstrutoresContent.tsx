...
function applyFilters(instructors: InstructorCard[], filters: InstructorSearchFilters, query: string) {
  ...
  if (filters.neighborhood) {
    results = results.filter(i => i.neighborhood.toLowerCase().includes(filters.neighborhood!.toLowerCase()))
  }
...
function InstrutoresContentInner({ instructors }: { instructors: InstructorCard[] }) {
  ...
  const [filters, setFilters] = useState<InstructorSearchFilters>({
    ...
    latitude: initialLatitude ? Number(initialLatitude) : undefined,
    longitude: initialLongitude ? Number(initialLongitude) : undefined,
  })
  const [showFilters, setShowFilters] = useState(false)
  const [viewMode, setViewMode] = useState<'grid' | 'map'>('grid')

  const filtered = applyFilters(instructors, filters, query)
            ...
            <div className="flex items-center gap-3">
              <form onSubmit={handleSearch} className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                {query && (
                  <button
                    type="button"
            ...
            <div className="min-w-0 flex-1">
              ...
              {viewMode === 'grid' ? (
                  ...
                  {filtered.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                      {filtered.map(instructor => (
                        <InstructorCardComponent key={instructor.id} instructor={instructor} />
                      ))}
                    </div>
                  )}
                </>
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-200 shadow">
                  <InstructorMap instructors={filtered} height="600px" />
                </div>
              )}
            </div>
          </div>
...
export default function InstrutoresContent({ instructors }: { instructors: InstructorCard[] }) {
  return (
    <Suspense>
      <InstrutoresContentInner instructors={instructors} />
    </Suspense>
  )
}
