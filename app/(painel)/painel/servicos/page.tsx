...
const CATEGORIES = [
  ...
  { value: 'C', label: 'Categoria C', desc: 'Veículos pesados', priceKey: 'price_per_lesson_c' },
  { value: 'D', label: 'Categoria D', desc: 'Transporte de passageiros', priceKey: 'price_per_lesson_d' },
  { value: 'E', label: 'Categoria E', desc: 'Combinação de veículos', priceKey: 'price_per_lesson_e' },
]

const LESSON_TYPE_OPTIONS = [
  { value: 'exam_prep', label: 'Preparação para exame' },
...
export default function ServicosPage() {
    ...
    const load = async () => {
      try {
        ...
        CATEGORIES.forEach(({ value, priceKey }) => {
          const v = (data as Record<string, unknown>)?.[priceKey] ?? data?.price_per_lesson
          if (v != null) initialPrices[value] = String(Number(v))
        })
        setPrices(initialPrices)
        setAcceptsStudentCar(Boolean(data?.accepts_student_car))

        const rawLT = data?.lesson_types
        if (Array.isArray(rawLT)) setLessonTypes(rawLT.map(String))
      } catch (err) {
  ...
  const toggleCategory = (cat: string) => {
    ...
    setPriceErrors(prev => { const n = { ...prev }; delete n[cat]; return n })
  }

  const toggleLessonType = (value: string) => {
    setLessonTypes(prev => prev.includes(value) ? prev.filter(t => t !== value) : [...prev, value])
  }
  ...
  const handleSave = async () => {
    ...
    selectedCategories.forEach(cat => {
      ...
      if (!v || v < 1) errors[cat] = 'Informe um valor de no mínimo R$ 1'
    })
    ...
    try {
      ...
      const allPrices = selectedCategories.map(c => Number(prices[c])).filter(p => p >= 1)
      ...
      toast.error((err as Error).message || 'Erro ao salvar serviços.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <div>
        <h1 className="text-2xl font-extrabold text-gray-900">Meus Serviços</h1>
        <p className="mt-1 text-sm text-gray-500">Gerencie as categorias que você ensina e os valores por aula.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-4">
            <div>
              <h2 className="text-base font-bold text-gray-900">Categorias que você ensina</h2>
              <p className="text-sm text-gray-500 mt-0.5">Selecione uma ou mais e informe o valor por aula de cada.</p>
            </div>
                    ...
                    {selected && (
                      <div className="px-4 pb-4">
                        ...
                        <input
                          ...
                          value={prices[value] ?? ''}
                          onChange={e => {
                            setPrices(prev => ({ ...prev, [value]: e.target.value }))
                            setPriceErrors(prev => { const n = { ...prev }; delete n[value]; return n })
                          }}
            ...
            <label className="flex items-center gap-3 cursor-pointer select-none pt-1">
              <button type="button" onClick={() => setAcceptsStudentCar(v => !v)} className="text-blue-600">
                {acceptsStudentCar
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">Selecione todas as modalidades que você atende.</p>
            </div>
              ...
              {LESSON_TYPE_OPTIONS.map(({ value, label }) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-xl border-2 px-4 py-3 text-sm font-medium text-left transition-colors ${
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-700 py-3 font-bold text-white hover:bg-blue-800 disabled:opacity-50 transition-colors"
          >
            {saving ? <><Loader2 className="h-4 w-4 animate-spin" /> Salvando...</> : 'Salvar serviços'}
          </button>
        </div>
      )}
    </div>
