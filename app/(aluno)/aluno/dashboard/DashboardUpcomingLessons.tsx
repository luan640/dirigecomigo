...
export default function DashboardUpcomingLessons({ bookings }: { bookings: UIBooking[] }) {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      const whatsappText = `Ola quero cancelar a aula (${selectedBooking.id}), por motivo (${trimmedReason}).`
      const whatsappUrl =
  ...
  return (
    <>
      {bookings.map(booking => (
        <div key={booking.id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-gray-900">{booking.instructor.name}</p>
            ...
            {booking.cancellation_reason && (
              <p className="mt-1 text-xs text-rose-600">Motivo informado: {booking.cancellation_reason}</p>
            )}
          </div>
          <div className="flex flex-col gap-2 sm:items-end">
            <div className="text-left sm:text-right">
              ...
              <p className="mt-1 text-xs font-bold text-gray-900">{formatCurrency(booking.gross_amount)}</p>
            </div>
            {canCancelBooking(booking) && (
              <button
                type="button"
                onClick={() => setSelectedBooking(booking)}
                className="inline-flex items-center justify-center gap-1 rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:bg-rose-100"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Ao confirmar, a aula sera cancelada para voce e para o instrutor, e o suporte entrara em contato para realizar o reembolso.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              ...
              O WhatsApp sera aberto para registrar a solicitacao.
            </div>
